...
- **L'autenticazione e l'amministrazione sono in codice, ma il sito non è ancora online.** Resta un passo manuale, tracciato in `HANDOFF.md`: su Vercel impostare la Root Directory su `web` e incollare le variabili d'ambiente (comprese `ADMIN_EMAIL` e `CRON_SECRET`)
