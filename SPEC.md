...
6. **Bot** — valutazione e scambi. 🚧 *in corso*, in `/jobs` (`valutazione.ts`, `scambi.ts`) e `web/app/squadre/[id]/scambi`. Fatto: proposta/accettazione/rifiuto/ritiro con storico persistito (`StatoLega.scambi`); valutazione di un giocatore come quota del budget della sua squadra, dal prezzo d'asta mano a mano sostituito dalla resa osservata (§7.1 punti 1–2), con correzione di scarsità del ruolo Mantra (punto 3); personalità e rumore per bot, stabili per stagione perché seminati su (seme di lega, squadra); anti-exploit — rumore, margine richiesto, soglia di rifiuto automatico, tetto stagionale — **solo quando la controparte è un bot** (decisione del proprietario: fra due persone uno scambio passa sempre). Non ancora fatto: correzione per buchi in rosa e crediti residui (punto 3 restante, aspetta l'asta nativa perché oggi i crediti non si spendono più a stagione aperta), bot che propongono per primi (oggi rispondono soltanto), il livello sociale di §7.2. Non testato in un browser reale, per lo stesso motivo di sempre: vedi `HANDOFF.md`
7. **Strato AI** — cronache, editoriale, chat. 🚧 *in corso*, in `/jobs/src/ai` (`provider.ts`, `cronaca.ts`, `editoriale.ts`) e `narrativa.ts`. Fatto: i primi due generatori di §8, dentro il job serale, con Groq come provider (interfaccia unica, `providerDallAmbiente` restituisce `null` senza chiave e tutto ricade sul template). La cronaca ha la validazione automatica che la specifica chiede — risultato esatto e marcatori citati — con un tentativo di rigenerazione; l'editoriale ha solo il fallback, perché la specifica lo chiede stretto solo per la cronaca. Provato con chiamate vere, non solo con un provider finto nei test: **il limite del piano gratuito che conta è il token al minuto, non le 15 chiamate al giorno stimate qui sopra** — una giornata da dieci cronache più un editoriale, generata tutta insieme, può esaurirlo a metà e cadere sul template per il resto. Non è un errore che blocca: è esattamente il fallback previsto, attivato più spesso del previsto. Non fatto: voci di mercato (aspetta la finestra di fine stagione, non ancora raggiungibile) e chat dei bot (aspetta una scheda personaggio non ancora progettata)
...
- Correzione di valutazione per buchi in rosa e crediti residui (SPEC 7.1 punto 3, parte restante): non implementata. I crediti residui non hanno un uso reale finché il mercato intra-stagione resta chiuso (SPEC 6.5) e non esiste l'asta nativa; da riprendere quando arriva quella milestone, probabilmente riusando la stessa `valutazione.ts`
- I bot oggi **rispondono** alle proposte di scambio ma non ne fanno mai per primi. Proporre richiede una decisione di prodotto in più — con quale frequenza, verso chi, per colmare quale buco — che conviene prendere osservando come si comportano le proposte umane in una stagione vera
- Taratura dei parametri di `fanta/config/scambi.json` (rumore, margine, soglia, tetto, scarsità di ruolo): valori di partenza plausibili, non ancora provati su una stagione giocata da persone vere
