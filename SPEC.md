...
1. **Assegnazione diretta**: l'amministratore della lega inserisce la mail di una persona su una squadra, da `/leghe/[legaId]/admin`. Chi si autentica con quella mail vede quella lega e quella squadra.
...
### Navigazione (issue #16)

**La home non mostra più niente da sola**: smista soltanto, verso `/login` se non c'è sessione (senza Supabase configurato si passa dritti, come per l'autenticazione) o verso `/leghe` altrimenti.

**`/leghe` — "le mie leghe"**: l'elenco delle leghe di chi ha fatto login, con azioni per entrare in una lega esistente (`/entra`) o crearne una (`/admin`, solo per chi può). Non serve una tabella di appartenenza dedicata: la policy RLS `"leghe: le proprie"` (§9, `e_della_lega`) filtra già `archivioPerRichiesta().elenca()` per proprietà di una squadra. L'unico caso che la RLS non copre è l'amministratore che non gioca lui stesso: per quello `legheDiUtente()` (`web/src/dati.ts`) scavalca la RLS con `archivioServizio` e filtra esplicitamente per la sua mail — lo stesso precedente già in uso per `/leghe/[legaId]/admin`, non una seconda nozione di appartenenza.

**`/leghe/[legaId]/…` — dentro una lega**: un menu con Dashboard, Rose, Calendario, Classifica e Sala stampa (più Amministrazione, verso `/leghe/[legaId]/admin`, se si amministra quella lega). `legaAutorizzata(legaId)` (`web/src/dati.ts`) sostituisce la vecchia `legaPredefinita()` ("la prima lega trovata", senza nessun legame con chi guarda) in ogni pagina qui sotto: legge con la chiave di servizio e verifica esplicitamente che chi chiede ci giochi o la amministri, cosi' un amministratore senza squadra propria non resta escluso come lo escluderebbe la RLS da sola.

Le vecchie route piatte (`/squadre`, `/giornate`) sono state ritirate, non lasciate come alias: è un'app privata per dieci amici senza link esterni da preservare.

**Contenuto delle schermate**, rivisto sul modello di Fantacalcio.it (solo struttura e passaggi fra pagine, non l'aspetto: nessuna immagine/logo di club o giocatori reali, regola 2).

Calendario a tre livelli: `/leghe/[legaId]/calendario` è la panoramica, dalla giornata scelta (o quella in corso, di default) fino a fine campionato, senza finestra ne' paginazione; `/leghe/[legaId]/calendario/[n]` fa solo da redirect verso il primo incontro della giornata (la propria squadra, se ne hai una); `/leghe/[legaId]/calendario/[n]/[squadraId]` è la pagina vera, con gli incontri della giornata a sinistra (come la barra delle squadre in Rose) e il dettaglio di quello scelto a destra — le due formazioni schierate, e per ogni giocatore le icone di bonus/malus (⚽ gol, 🎯 rigore segnato, 👟 assist, 🧤 rigore parato, ❌ rigore sbagliato, 🔴 autogol, 🟨 ammonizione, 🟥 espulsione) al posto del numero netto, riprese da `eventiDaPrestazione` (`jobs/src/ciclo.ts`, già usata dal motore fanta) applicata alle prestazioni del mondo di quella giornata — non serve nessun nuovo campo salvato. Editoriale, cronache dal campionato, chat e "cosa è successo alle formazioni" (contenuti nostri, l'app di riferimento non li ha) restano sotto, per tutta la giornata.

La rosa (`/leghe/[legaId]/rose/[id]`) ha una barra laterale per passare da una squadra all'altra, le colonne MV (media voto del mondo, `mediaVoto` di `jobs/src/valutazione.ts`) e FM (fantamedia, aggregata in `mvFmDiRosa`, `web/src/dati.ts`), ed export CSV. La classifica ha differenza reti e fantamedia fatta/subita in più.

**Prestazioni**: `legaAutorizzata` (`web/src/dati.ts`) è avvolta in `cache()` di React — il layout di `/leghe/[legaId]` e la pagina sotto la chiamavano entrambi, raddoppiando inutilmente le nove query a Supabase di `archivioServizio.leggi` a ogni click. Ogni pagina di lega ha anche un `loading.tsx` condiviso, per un riscontro immediato mentre i dati (e il ricalcolo della stagione, che costa "qualche decimo di secondo" per costruzione, §9) sono pronti.
