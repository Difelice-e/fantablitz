...
**Contenuto delle schermate**, rivisto sul modello di Fantacalcio.it (solo struttura e passaggi fra pagine, non l'aspetto: nessuna immagine/logo di club o giocatori reali, regola 2). Il calendario di una giornata ha due livelli: `/leghe/[legaId]/calendario/[n]` elenca gli incontri (più editoriale, cronache e chat, contenuti nostri che l'app di riferimento non ha), e ogni incontro rimanda a `/leghe/[legaId]/calendario/[n]/[squadraId]` con le due formazioni schierate e bonus/malus per giocatore. La rosa (`/leghe/[legaId]/rose/[id]`) ha una barra laterale per passare da una squadra all'altra, le colonne MV (media voto del mondo, `mediaVoto` di `jobs/src/valutazione.ts`) e FM (fantamedia, aggregata in `mvFmDiRosa`, `web/src/dati.ts`), ed export CSV. La classifica ha differenza reti e fantamedia fatta/subita in più.
